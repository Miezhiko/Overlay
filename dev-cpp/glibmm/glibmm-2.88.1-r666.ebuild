...
PATCHES=(
	...
	# Backport of upstream fixes for building against glib >= 2.89.2, which
	# declares GDBusActionGroup/GEmblem as G_DECLARE_FINAL_TYPE and conflicts
	# with glibmm's own unconditional class-name aliases otherwise.
	"${FILESDIR}"/${PN}-2.88.1-gdbusactiongroup-emblem-final-types.patch
	"${FILESDIR}"/${PN}-2.88.1-dbusactiongroup-final-type-check.patch
)
