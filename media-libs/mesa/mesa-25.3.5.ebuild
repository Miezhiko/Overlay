...
src_prepare() {
	...
	# some old gentoo bug from gentoo ebuild
	sed -i -e "/^PLATFORM_SYMBOLS/a '__gentoo_check_ldflags__'," \
		bin/symbols-check.py || die # bug #830728

	# Fix const-correctness error in nir_load_libclc.c
	sed -i 's/char \*U3AS1 = strstr/const char *U3AS1 = strstr/' \
		"${S}"/src/compiler/clc/nir_load_libclc.c || die "U3AS1 bug sed failed"
}
