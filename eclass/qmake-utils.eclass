...
#
# This eclass unconditionally inherits qt-utils.eclass and all its public
# helper functions may be considered as part of this eclass's API.
#
...
qt5_get_qmake_args() {
	cat <<-EOF
		...
		# qmake's link rules only ever pull in QMAKE_LFLAGS, never
		# QMAKE_C(XX)FLAGS. When C(XX)FLAGS enable LTO (e.g. -flto=thin),
		# object files are emitted as LLVM bitcode, and a plugin-aware
		# linker (e.g. mold) then refuses to link them without an
		# explicit -flto on the link line:
		#   mold: fatal: ...: unable to handle this LTO object file
		#   because the -plugin option was not provided.
		# Fold CXXFLAGS into the link flags so the link step gets the
		# same optimization/LTO flags as the compile step.
		QMAKE_LFLAGS="${LDFLAGS} ${CXXFLAGS}"
