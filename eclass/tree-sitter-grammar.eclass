...
tree-sitter-grammar_src_compile() {
	...
	if [[ -f "${S}/pyproject.toml" ]]; then
		...
		# Upstream's Makefile links the shared library with $(LDFLAGS) only,
		# never $(CFLAGS). When CFLAGS enables LTO (e.g. -flto=thin), the
		# compiled .o files are LLVM bitcode, and a plugin-aware linker
		# (e.g. mold) then refuses to link them without an explicit -flto
		# on the link line:
		#   mold: fatal: ...: unable to handle this LTO object file
		#   because the -plugin option was not provided.
		# Fold CFLAGS into the LDFLAGS make var we pass in, so the link
		# step gets the same optimization/LTO flags as the compile step.
		emake \
			CC="$(tc-getCC)" \
			AR="$(tc-getAR)" \
			STRIP="" \
