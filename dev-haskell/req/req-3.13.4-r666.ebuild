...
CABAL_CHDEPS=(
	# Local overlay's http-client-tls is now 0.4.0 (API-compatible; only
	# an internal MD5-hashing dependency swap, see that package's own
	# comments). Relax this package's cabal-declared upper bound to match.
	'http-client-tls >=0.3.2 && <0.4' 'http-client-tls >=0.3.2 && <0.5'
)
...
RDEPEND=">=dev-haskell/aeson-0.9:=[profile?] <dev-haskell/aeson-3:=[profile?]
	...
	>=dev-haskell/crypton-connection-0.4.2:=[profile?] <dev-haskell/crypton-connection-0.5:=[profile?]
	>=dev-haskell/data-default-class-0.2.0.0:=[profile?]
	...
	>=dev-haskell/http-client-tls-0.3.2:=[profile?] <dev-haskell/http-client-tls-0.5:=[profile?]
