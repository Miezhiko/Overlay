...
RDEPEND="
	...
	>=dev-libs/template-glib-3.38.0:=[introspection]
...
src_prepare() {
	...
	find . -type f -name "meson.build" -exec sed -i 's/-Werror//g' {} + || die
	find . -type f -name "*.build" -exec sed -i 's/-Werror//g' {} + || die
	find . -type f -name "build.ninja" -exec sed -i 's/-Werror//g' {} + || die
}
